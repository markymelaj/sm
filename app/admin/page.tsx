...
export default async function AdminDashboardPage() {
  ...
  const [{ data: cuotasData }, { count: clientesConAcceso = 0 }, { count: clientesSinAlta = 0 }, { count: solicitudesActivas = 0 }, { count: solicitudesAccesoPendientes = 0 }] = await Promise.all([
    ...
    createAdminClient().from('solicitudes_acceso').select('*', { count: 'exact', head: true }).eq('estado', 'pendiente')
  ]);
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <div className="card p-5"><p className="text-sm font-bold uppercase tracking-[0.22em] text-amber-300">Solicitudes acceso</p><h2 className="mt-2 text-2xl font-bold text-white">{solicitudesAccesoPendientes}</h2></div>
      </section>
      <section className="card grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-5">
        ...
        <Link className="rounded-2xl border border-white/8 bg-slate-900/45 p-4" href="/admin/solicitudes-acceso"><p className="text-lg font-bold text-white">Solicitudes de acceso</p><p className="muted mt-2 text-sm">Aprueba o rechaza nuevos ingresos al portal.</p></Link>
