...
const TABS = [
  ...
  { key: 'admin', label: 'Cargados por admin' },
...
export default async function AdminPagosPage({
  ...
  const cuotas = sortQuotasByUpdatedDesc((cuotasData ?? []) as any[])
    ...
    .filter((cuota: any) => {
      ...
      if (normalizedTab === 'admin') return cuota.origen === 'admin';
    ...
    <section className="card p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="muted mt-2 text-sm">Revisa pagos aprobados, rechazados y el origen del registro.</p>
        </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {TABS.map((item) => (
          <a
            key={item.key}
            className={`rounded-full border px-4 py-2 text-sm font-semibold ${normalizedTab === item.key ? 'border-amber-300/40 bg-amber-400/15 text-amber-200' : 'border-white/10 bg-slate-900/45 text-slate-200'}`}
            href={`/admin/pagos?tab=${item.key}${q ? `&q=${encodeURIComponent(q)}` : ''}`}
          >
            {item.label}
          </a>
        ))}
