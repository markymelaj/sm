      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        {cuotas.map((cuota: any) => (
          <article className="rounded-2xl border border-white/8 bg-slate-900/45 p-4" key={cuota.id}>
            <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg font-bold text-white">{cuota.concepto}</h3><p className="muted text-sm">{cuota.perfiles?.nombre_completo || 'Cliente'} · Fecha de pago {formatDate(cuota.fecha_vencimiento)}</p></div><StatusBadge label={cuota.estado} /></div>
